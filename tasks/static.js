...
gulp.task('copyStatic', function() {
	return gulp.src([config.client + '**/*.html', config.client + '**/*.js', config.assetsPath.fonts + '**/*.*', config.assetsPath.images + '**/*.*'], {base: config.client})
	    .pipe(gulp.dest(config.build_dev.path));
});
gulp.task('copyVendor', function() {
	return gulp.src(requireJS, {base: './node_modules'})
